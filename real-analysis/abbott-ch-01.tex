...
Abbott begins by introducing us to some core concepts:

Natural numbers are $\mathbb{N} = \{1, 2, 3, \ldots\}$.

Integers are $\mathbb{Z} = \{\ldots, -1, 0, 1, \ldots\}$.

Rational numbers are $\mathbb{Q} = \left\{ \frac{p}{q} : p, q \in \mathbb{Z}, q \neq 0\right\}$.

The integrers give us a aditive identity (0) and the additive inverse necessary for subtraction.
The rationals give us the multiplicative identity (1) and the multiplicative inverse necesary for division.
All the above combined give us a \textbf{field}.
